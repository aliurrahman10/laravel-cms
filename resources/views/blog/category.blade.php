...
{{--             <p class="lead-2 opacity-90 mt-6">{{$latest->title}}</p> --}}
              ...
              <div class="row gap-y">
                ...
                <div class="col-md-6">
                  <div class="card border hover-shadow-6 mb-6 d-block">
                    <a href="{{route('posts.single-post', $post->id)}}"><img class="card-img-top" src="{{asset('storage/'.$post->image)}}" alt="Card image cap"></a>
                    <div class="p-6 text-justify">
                      <p><a class="small-2  text-uppercase ls-2 fw-700" href="{{route('posts.single-post',$post->id)}}">{{$post->title}}</a></p>
                      <h5 class="mb-0">
                        <a class="text-dark" href="{{route('posts.single-post',$post->id)}}">{{$post->description}}</a>
                      </h5>
                    </div>
                  </div>
                </div>
                ...
                @empty
                  <p class="text-center">No result found for this search <strong>{{ request()->query('search') }}</strong></p>
                @endforelse
